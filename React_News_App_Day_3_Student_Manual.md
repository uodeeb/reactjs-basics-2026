...
# Day 3 Student Manual
# The Flow Gate — Turn It Into an Application

> **Project:** React News App  
> **Gate:** The Flow Gate
> **Beginner question:** How does the URL become a page, and a page become navigation?
> **Day 3 focus:** One-way data flow, lifting state, React Router, route parameters, article details, custom Hooks, and refactoring
> **Learning cycle:** Need → Try → Fail → Observe → Explain → Fix → Refactor → Apply

---

## Your Quest Map for Day 3

The Data Gate delivered real data into the app. Now the ninja asks the next question:

> "How do I turn this single screen into a whole application — pages, navigation, deep links?"

Today you cross the **Flow Gate**. By the end of the day you should control this chain:

```text
One-way data flow
   ↓
State lives in the right place (lifting state)
   ↓
...
Duplicated logic → custom Hooks
   ↓
A clearly organized growing app
```
...
The official React course outline includes React basics, reusable components, forms and validation, React Router, one-way data flow, HTTP/API calls, Hooks, custom Hooks, and Context API. This day focuses especially on **one-way data flow, routing, route parameters, article details, custom Hooks, and refactoring**.
...
> **Tagline:** Data flows down; news flows up.
...
Routing with React Router is part of the official course curriculum.
...
> **Tagline:** The router turns the URL into a state you can read.
...
### Deeper: Route Parameters Are Always Strings

Notice the quotes: `useParams()` returns the parameter as a **string**, because the URL itself is a string.

```text
URL:   /article/25
         ↓
param: "25"   (a string, not a number)
```

This is exactly the shape of the bug you will meet in section 29:

```jsx
const { id } = useParams();          // "25"  (string)
article.id === id                    // 25 === "25" → false!
```

If your data has numeric `id`s, comparing `article.id === id` fails silently. Convert before comparing:
...
`Number("25")` is `25`, and now the comparison works. Remember the rule: **the URL gives you text — you decide when to treat it as a number.**

> **Tagline:** The URL is the address of the page; the param is the part that changes — and it arrives as a string.
...
> **Tagline:** A custom Hook is a repeated behavior you gave a name.
...
# 58. Day 3 Summary — The Staircase of the Flow Gate

Today you moved from individual React features toward application architecture.

At the start of the day, the app was a single screen with features. Now the stair behind you is visible:
...
> **Tagline:** The router turns the URL into a state you can read — and you learned to read it.

## Next Gate: The Architecture Gate

The Flow Gate is closed. Tomorrow the ninja enters the final gate of the React module, the **Architecture Gate**:

> *"How do I keep a growing app clean and share state sanely?"* The temple rule waits: every new tool answers an old question — and the Architecture Gate's tool, Context, answers the question left behind by prop drilling.

---

# Using AI at the Flow Gate

AI can explain React Router, show you the difference between `Link` and `useNavigate`, or walk through why a route renders nothing. Use it as a sparring partner, not a substitute.

**Good prompt:**

> "My `/article/:id` page renders nothing and the URL looks right. Give me hints on what to check first, then explain."

**Bad prompt:**

> "Make my routing work."

The rule of the temple:

> **You learn by explaining the fix yourself.** If you cannot trace the URL from the link to the rendered page, the gate is not yet closed.

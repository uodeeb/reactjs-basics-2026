...
# The Components Gate — React Changes How We Build UI

> **Project:** React News App  
> **Gate:** The Components Gate
> **Beginner question:** How do I stop UI chaos from multiplying?
...
## Your Quest Map for Day 1

You have walked the earlier gates of the temple:

```text
HTML            → structure
CSS             → presentation
JavaScript      → behavior
DOM + Events    → change the page
APIs            → get data
Git + DevTools  → work safely
ES6 + npm + Vite → organize larger projects
```

Today you enter the **Components Gate**. By the end of the day you should control this chain:
...
Every concept in that chain answers a question left behind by the previous step — that is the temple rule:
...
## 3b. How We Got to React (30-Second Bridge)

In the React Gate you climbed the staircase that leads here. One breath, in case it faded:

```text
The web was documents (HTML + CSS)
   ↓
JavaScript added behavior, and the DOM allowed page changes
   ↓
Big apps made hand-written DOM updates painful
   ↓
Component-based UI libraries answered: React
   ↓
JSX needs compiling → Node.js, npm, and Vite provide the tooling
   ↓
Today: you build components, the tools do the rest
```

> **Tagline:** React did not appear alone — and today it is your turn to build with it.
...
> **Tagline:** Props are the answers a parent sends to a child.
...
> **Tagline:** A list is one template, repeated many times — never many copies.
...
### Deeper: What Actually Happens Without a Stable Key?

When a list re-renders, React needs to know which item from the previous render matches which item in the new render. This matching is called **reconciliation** — React figures out the smallest amount of UI to update.

Without a key, React falls back to matching items by their **position** in the array. That works for a static list. The moment the list changes:

```text
Before (favorite on item B):

[A]  [B♥]  [C]

New item X inserted at the top:

[X]  [A]  [B♥]  [C]
```

If React matches by position, it may think the old [A] became the new [X], the old [B♥] became the new [A], and so on. The favorite can end up on the wrong article — or the article's internal state can be lost.

A stable `key={article.id}` tells React:

> "This item is the same article as before. Keep its state; just move it."

So the key is not styling. It is **identity**. That is why your favorite stays with the right article across re-renders on Day 3, when the list changes more often.

> **Tagline:** React needs a name tag for each item, not for decoration.
...
> **Tagline:** State is the memory of the interface.
...
> **Tagline:** Events turn static content into interaction.
...
# 53. The Staircase You Climbed Today

At the start of the day, the News App was an empty project and a vague idea. Look at the stair behind you:
...
> **Tagline:** Components turn a crowd of code into a team of small, clear units — and today you made your first team.

---

# 54. The Next Gate: The Data Gate
...
> **Next quest:** Tomorrow you enter the **Data Gate** — *"How does my app get data from the outside world?"* The temple rule applies there too: every new tool answers an old question. The Data Gate's tools answer the question left behind by local sample data.
...
# Using AI at the Components Gate

AI is a sparring partner, not a substitute. Today, use it to explain — not to write the code you should be writing.

**Good prompt:**

> "My NewsCard renders nothing and the Console says `Cannot read properties of undefined (reading 'title')`. Give me hints on what to inspect first, then explain why."
...
> **You learn by explaining the fix yourself.** If you cannot explain why the fix works, the gate is not yet closed.
